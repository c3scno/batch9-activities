...
const formOpponentSelection = document.querySelector(
...
function stampMark() {
  ...
  if (this.textContent === "") {
    ...
    turnNo++;
    console.log(
      `Move ${turnNo}: ${currentMark} to row# ${
        parseInt(this.dataset.row) + 1
      } & col# ${parseInt(this.dataset.col) + 1} `
    );
    let history = document.getElementById("history");
    history.innerHTML += `
            <p>
                Move ${turnNo}: ${currentMark} to Row no.${
      parseInt(this.dataset.row) + 1
    } Col no.${parseInt(this.dataset.col) + 1} 
            </p>
        `;
...
function computerEasy() {
  ...
  let emptyIndexes = indexes.filter(
    (index) => gridItems[index].textContent === ""
  ); // create new array of indexes that pass the test
  let selectedIndex =
    emptyIndexes[Math.floor(Math.random() * emptyIndexes.length)]; // get a random no text gridItem
  ...
  history.innerHTML += `
        <p>
            Move ${turnNo}: ${currentMark} to Row no.${
    parseInt(randomGridItem.dataset.row) + 1
  } Col no.${parseInt(randomGridItem.dataset.col) + 1} 
        </p>
...
function switchStates() {
  ...
  if (isGameActive === true) {
    ...
    currentPlayerMessage =
      currentMark === inputX ? `${xName} (${inputX})` : `${oName} (${inputO})`;
...
function resetBoard() {
  isGameActive = true;
  if (currentMark === inputX) {
    // if x wins; let o go first next
    playerOMovesFirst();
  } else {
    // else if o wins; let x go first next
    currentMark = inputX;
